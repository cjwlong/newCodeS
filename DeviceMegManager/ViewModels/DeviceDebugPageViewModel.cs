...
{
   public class DeviceDebugPageViewModel : BindableBase
    {

        public DeviceDebugPageViewModel(IContainerProvider provider)
        {
        ...
        public DelegateCommand<string> SaveSettingsCommand => _saveSettingsCommand ??
            (_saveSettingsCommand = new DelegateCommand<string>((r) =>
            {
                ...
                if (info != "") eventAggregator.GetEvent<SaveDebugEvent>().Publish(info);
            }));
    }
